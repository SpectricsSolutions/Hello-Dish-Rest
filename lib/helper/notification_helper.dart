...
import 'package:sixam_mart_store/main.dart';
...
class NotificationHelper {
  ...
  static String? _currentRingingOrderId;
  ...
  static Future<void> stopOrderAlert({int? orderId}) async {
    ...
    if(orderId != null && _currentRingingOrderId != null && _currentRingingOrderId != orderId.toString()) {
      return;
    }
    await stopService();
  }

  static Future<void> stopRingingForOrder(int? orderId) => stopOrderAlert(orderId: orderId);
...
Future<dynamic> myBackgroundMessageHandler(RemoteMessage message) async {
  ...
  if(notificationBody.notificationType == NotificationType.order) {
    ...
    NotificationHelper._currentRingingOrderId = notificationBody.orderId?.toString();
...
Future<ServiceRequestResult> _startService(String? orderId) async {
  NotificationHelper._currentRingingOrderId = orderId;
...
Future<ServiceRequestResult> stopService() async {
  ...
  NotificationHelper._currentRingingOrderId = null;
...
class MyTaskHandler extends TaskHandler {
  ...
  void _playAudio() async {
    try {
      ...
      await _audioPlayer.stop();
      // Play once, not in loop - let onRepeatEvent handle repetition
